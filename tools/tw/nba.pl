...
For Login
   shell > ./nba.pl  --host twnba_login1 twnba_login2  -t login -p 8200 -o start
=================================================================
EOF
exit;
}
...
$ssh->error and warn "can not connect to $host1" . $ssh->error;
		...
		if($t3 =~ /^gs$/)
		{
			...
			for(my $i=0;$i<$t2;$i++){
				my $pt=$t5+$i;		
        			print "World:	$t1"."\n";
				print "Port:	$pt"."\n";
				$ssh->system("/sbin/initctl start gs_8600 PORT=$pt WORLD=$t1 TARGET=10");
				}
        		}
		elsif($t3 =~ /^ca$/) 
		{
		...
		if($t3 =~ /^gs$/)
		{
        		print "$host1:"."\n";
        	                for(my $i=0;$i<$t2;$i++){
                                ...
                                print "World:   $t1"."\n";
                                print "Port:    $pt"."\n";
				 $ssh->system("/sbin/initctl stop gs_8600 PORT=$pt WORLD=$t1 TARGET=10");
                                }
		}
		elsif($t3 =~ /^ca$/) 
		{
	...
	elsif($t4 =~ /^restart$/)
	{
		if($t3 =~ /^gs$/)
		{
		        print "$host1:"."\n";
                        for(my $i=0;$i<$t2;$i++){
                                ...
                                print "World:   $t1"."\n";
                                print "Port:    $pt"."\n";
                              #  $ssh->system("/sbin/initctl stop gs_8600 PORT=$pt WORLD=$t1 TARGET=10 && /sbin/initctl start gs_8600 PORT=$pt WORLD=$t1 TARGET=10");
		...
		{
        	print "$host1:"."\n";
        #	$ssh->system("/sbin/initctl stop calculation_8400 && /sbin/initctl start calculation_8400") ;
...
if ($t2 == 8300)
{
	...
	{	printf $host2.":"."\n";
		...
		$ssh->system("initctl start login_8300 PORT=$j TARGET=10") ;
		}
	}
	elsif($t1 =~ /^stop$/)
	{	printf $host2.":"."\n";
		for(my $j=8301;$j<8311;$j++){
                $ssh->system("initctl stop login_8300 PORT=$j TARGET=10") ;
                }
	}
	elsif($t1 =~ /^restart$/)
	{	printf $host2.":"."\n";
		for(my $j=8301;$j<8311;$j++){
                $ssh->system("initctl stop login_8300 PORT=$j TARGET=10 && initctl start login_8300 PORT=$j TARGET=10") ;
		}
	}
	else 
	{
...
elsif($t2==8200)
{
 if($t1 =~ /^start$/)
        {       printf $host2.":"."\n";
                ...
                $ssh->system("initctl start login_8200 PORT=$j TARGET=10") ;
                }
        }
        elsif($t1 =~ /^stop$/)
        {       printf $host2.":"."\n";
                for(my $j=8201;$j<8211;$j++){
                $ssh->system("initctl stop login_8200 PORT=$j TARGET=10") ;
                }
        }
        elsif($t1 =~ /^restart$/)
        {       printf $host2.":"."\n";
                for(my $j=8201;$j<8211;$j++){
                #$ssh->system("initctl stop login_8200 PORT=$j TARGET=10 && initctl start login_8200 PORT=$j TARGET=10") ;
